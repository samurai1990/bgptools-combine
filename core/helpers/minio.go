...
func (m *Storage) checkBucket() error {
	if ok, err := m.MinioClient.BucketExists(context.Background(), m.MinioInfo.bucketName); err != nil {
		return fmt.Errorf("edge error: %s", err.Error())
	} else {
		if !ok {
			if err := m.MinioClient.MakeBucket(context.Background(), m.MinioInfo.bucketName, minio.MakeBucketOptions{}); err != nil {
				return fmt.Errorf("make bucket `%s` in edge error: %s", m.MinioInfo.bucketName, err.Error())
			}
		}
	}
	return nil
...
func (m *Storage) UploadToS3(path string) error {
	...
	if errU != nil {
		return fmt.Errorf(errU.Error())
	}
	log.Printf("upload file '%s' successed.", filename[len(filename)-1])
	os.Remove(path)
	return nil
}
...
func (m *Storage) GetS3() error {
	...
	for f, filename := range m.ListFiles {
		...
		go func(format, name string, e map[string]string) {
			...
			log.Printf("get file '%s' form S3 is successed.", name)
			mux.Lock()
...
func (m *Storage) httpTLlsChecker() {
	...
	_, err := client.Get(fmt.Sprintf("https://%s:%s", m.MinioInfo.ip, m.MinioInfo.port))
	if err == nil {
		m.MinioInfo.tls = true
	}
}
